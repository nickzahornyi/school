...
export class Lessons {
    ...
    async addVideo() {
        ...
        const data = await lessons.findOneAndUpdate({ hash }, { $addToSet: { 'content.videos': payload } }, { new: true });
    ...
    async addKeynote() {
        ...
        const data = await lessons.findOneAndUpdate({ hash }, { $addToSet: { 'content.keynotes': payload } }, { new: true });
    ...
    async getVideo() {
        ...
        if (!video) {
            throw new NotFoundError(`lesson with hash ${hash} does not contain video with hash ${videoHash}`);
        }
    ...
    async removeVideo() {
        ...
        if (!source) {
            throw new NotFoundError(`can not find lesson with hash ${hash} and video hash ${videoHash}`);
        }
    ...
    async getKeynote() {
        ...
        if (!keynote) {
            throw new NotFoundError(`lesson with hash ${hash} does not contain keynote with hash ${keynoteHash}`);
        }
    ...
    async removeKeynote() {
        ...
        if (!source) {
            throw new NotFoundError(`can not find lesson with hash ${hash} and keynote hash ${keynoteHash}`);
        }
